...
public class DynamicDataSourceRegister implements ImportBeanDefinitionRegistrar, EnvironmentAware
{
    ...
    private void initslaveDataSources(Environment env) {
        ...
        try
        {
            ...
            String main_db_config_table_name = env.getProperty("main_db_config_table_name");
            logger.debug("数据库配置表名称：" + main_db_config_table_name);
            PreparedStatement psts = connection.prepareStatement("select * FROM " + (StringUtils.isEmpty(main_db_config_table_name) ? "sys_db_config" : main_db_config_table_name));
            ...
            while (rs.next()){
                ...
                String driver_class_name = rs.getString(3);
                String url = rs.getString(4);
                String username = rs.getString(5);
                String password = rs.getString(6);
                String qknavy_db_prefix = env.getProperty("qknavy_db_prefix");
                if (StringUtils.isEmpty(qknavy_db_prefix)){
                    qknavy_db_prefix = "qknavy-db-";
                }
                logger.debug("数据库前缀：" + qknavy_db_prefix);
                map.put("type", type);
                map.put("driver-class-name", driver_class_name);
                map.put("url", url.replace("DATABASE_NAME",qknavy_db_prefix + id));
